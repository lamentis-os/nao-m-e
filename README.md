...
- Source-conditioned recall combines symbolic cue overlap with learned direct
  relevance without stored activation or mutation.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(hits[0].activation.as_ppm(), 640_000);
    Ok(())
}
```
...
```sh
printf '%s\n' \
  '--occurred 1785596520000 --recorded 1785596525000 --source 7 --predicate 22 --terms 2002' \
  '--occurred 1785596580000 --recorded 1785596585000 --source 7 --predicate 23 --terms 2003 --action 40:5001' \
...
This first recall has no relevance edge. The two episodes share only term
`1001`, whose cue weight is one; their total cue weights are 20 and 16. Weighted
Jaccard similarity therefore produces `floor(1 * 400000 / 35) = 11428` ppm.
...
the logical episode or relevance state; its first successful query with a
positive limit may initialize that private cache even when it finds no hits.

Helpful feedback then adds 1,000 ppm of direct relevance. Its projected learned
contribution is 400 ppm and is added to the unchanged structural score:
...
edges. It therefore cannot hide a cue-derived match after the learned edge
reaches zero. Feedback is deterministic and budgeted, does not recompute
recall, and does not bind the event to an earlier query. The next recall from
the same source observes the resulting graph. Each feedback command uses one
save, and the snapshot stores neither a feedback receipt nor provenance for the
change.
...
An initialized memory and newly added episodes have no relevance edges, but
recall can immediately return episodes sharing symbolic predicate or term cues.
Direct relevance can also introduce a target with no shared cue and feedback
can change later ranking. The CLI does not list all episodes, assign symbolic
